"""
Completes the 328 AEGIS examples needed to make Llama 3.3 70B use the EXACT
SAME canonical 400-example AEGIS set (200 unsafe / 200 safe) as the primary
7-8B experiments (llama3.1_8b, qwen2.5_7b -- confirmed identical IDs across
both; mistral_7b has no primary AEGIS pair). The existing llama_70b AEGIS
data (analysis/frontier_model/llama_70b/ + llama_70b_spark_local/, combined)
used a DIFFERENT 400-example set (a plain prefix slice, not the balanced
seeded sample), so only 72 of its 400 IDs are safely reusable -- and unlike
the HateModerate case, all 72 overlapping IDs agreed on ground truth (0
mismatches), so no additional exclusions were needed.

Driven by an EXPLICIT manifest (llama70b_aegis_missing_manifest.json, 328
entries: {"id":..., "groundTruth":...}) rather than a dataset diff, so the
exact set of IDs to run is fixed and auditable.
...
Writes to a SEPARATE directory (llama_70b_spark_aegis_matched/) -- does NOT
touch analysis/frontier_model/llama_70b/ or llama_70b_spark_local/ (the
original two AEGIS sources). Backend: local Ollama (GGUF quantization),
NOT HopGPT/Bedrock -- same deliberate, disclosed choice as the HateModerate
matched run.

Run this SEQUENTIALLY after (not concurrently with)
run_llama70b_spark_hatemoderate_matched.py on the same machine -- Spark
hosts a single Ollama instance, and this session already confirmed running
concurrent load against it causes real server-side degradation/stalls, not
just slower throughput.

Usage (run from the ICLR_2027 repo root, on Spark):
    python3 run_llama70b_spark_aegis_matched.py --model llama3.3:70b --resume
"""
...
MANIFEST_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              "analysis", "frontier_model", "llama70b_aegis_missing_manifest.json")

OUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                        "analysis", "frontier_model", "llama_70b_spark_aegis_matched")
...
def main():
    ...
    if not os.path.isfile(MANIFEST_PATH):
        sys.exit(f"ERROR: manifest not found at {MANIFEST_PATH} -- copy "
                  f"analysis/frontier_model/llama70b_aegis_missing_manifest.json onto Spark first.")
    ...
    if missingFromLoader:
        print(f"WARNING: {len(missingFromLoader)} manifest IDs not found in this machine's "
              f"AEGIS loader output -- dataset drift risk. First few: {missingFromLoader[:5]}")
    ...
    print(f"Manifest examples: {len(examples)} (target 328)")
